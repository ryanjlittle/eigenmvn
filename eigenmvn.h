...
{
  static osuCrypto::PRNG rng;                        // The uniform pseudo-random algorithm
  mutable std::normal_distribution<double> norm; // gaussian combinator

  EIGEN_EMPTY_STRUCT_CTOR(scalar_normal_dist_op)

  template<typename Index>
  inline const Scalar operator() (Index, Index = 0) const { return (Scalar) norm(rng); }
  inline void seed(const osuCrypto::block &s) { rng.SetSeed(s); }
};

template<typename Scalar>
osuCrypto::PRNG scalar_normal_dist_op<Scalar>::rng;
...
class EigenMultivariateNormal
{
 ...
 public:
  EigenMultivariateNormal(const Matrix<Scalar,Dynamic,1>& mean,const Matrix<Scalar,Dynamic,Dynamic>& covar,
                          const bool use_cholesky=false,const osuCrypto::block &seed=osuCrypto::sysRandomSeed())
  ...
  {
      Matrix<Scalar,Dynamic,Dynamic> foo =  Matrix<Scalar, Dynamic, -1>::NullaryExpr(_covar.rows(), nn, randN);
      return (_transform * foo).colwise() + _mean;
  }
